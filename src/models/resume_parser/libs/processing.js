...
PreparedFile.prototype.saveResume = function (savePath, cbSavedResume) {
  ...
  if (fs.statSync(savePath).isDirectory() && this.resume) {
    fs.writeFile(path.join(savePath, this.name + '.json'), this.resume.jsoned(), cbSavedResume);
  }
};
